...
class ResNet(nn.Module):
    def __init__(self, block, layers, instance_affine=False, num_classes=3, input_channels=1):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.weight.data.normal_(0, math.sqrt(2. / n)) # 0均值正态分布
...
def resnet34(pretrained=False, instance_affine=False):
    model = ResNet(BasicBlock, [3, 4, 6, 3], instance_affine=instance_affine)
    if pretrained:
...
def resnet50(pretrained=False, instance_affine=False):
    model = ResNet(Bottleneck, [3, 4, 6, 3], instance_affine=instance_affine)
    if pretrained:
...
def resnet101(pretrained=False, instance_affine=False):
    model = ResNet(Bottleneck, [3, 4, 23, 3], instance_affine=instance_affine)
    if pretrained:
...
def resnet152(pretrained=False, instance_affine=False):
    model = ResNet(Bottleneck, [3, 8, 36, 3], instance_affine=instance_affine)
    if pretrained:
...
if __name__ == '__main__':
    model = resnet18(pretrained=False, instance_affine=True)
